...
select t.trans_id, t.date, t.type, t.amount from trans as t
...
select district_id, count(client_id) as 'How Many' from client
...
select c.type, count(card_id) as How_many from card c
group by c.type
...
select l.date, count(loan_id) from loan l
where l.date < 930907
group by l.date
...
select l.date, l.duration, count(loan_id) as How_many from loan l
where l.date between 971201 and 97123
...
select account_id, t.type, round(sum(amount), 2) as total_amount from trans t
...
round(sum(amount), 0) as total_amount from trans t
where account_id = 396
group by t.type
order by t.type asc;
...
-- Query 20. From the previous result, modify your query so that it returns only one row, with a column for incoming amount, outgoing amount and the difference.
 select account_id, 
 case 
 when t.type = 'VYDAJ' then round(sum(amount), 0)  end as 'Incoming',
 case
 when t.type = 'PRIJEM' then round(sum(amount), 0) end as 'Outgoing', 
